  ...
  disposables.add(
    ...
    lumine.commands.add("lumine-workspace", {
      "line-ending-selector:show": async () => {
        ...
        // Read from the buffer rather than from the status tile: the tile only
        // exists once the status bar has been consumed, and the picker has to
        // work without one.
        await selector.show(await getLineEndings(editor.getBuffer()));
      },
    }),
    // The conversions stay editor-local: they act on the editor they were
    // dispatched in.
    lumine.commands.add("lumine-text-editor", {
      "line-ending-selector:convert-to-LF": (event) => {
        const editorElement = event.target.closest("lumine-text-editor");
...
module.exports = { activate, deactivate, consumeStatusBar, setLineEnding, getLineEndings };
